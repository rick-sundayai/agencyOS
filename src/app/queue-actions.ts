...
async function requireCanAct(id: string): Promise<{ userId: string }> {
...
async function transitionOrFriendlyError(
  id: string, to: 'approved' | 'cancelled', actor: string,
): Promise<DecisionRow> {
  try {
    return await transitionDecision(id, to, actor);
  } catch (err) {
...
export async function approveDecisionAction(id: string): Promise<DecisionRow> {
  const { userId } = await requireCanAct(id);
  const row = await transitionOrFriendlyError(id, 'approved', userId);
  revalidatePath('/');
  return row;
}
...
export async function cancelDecisionAction(id: string): Promise<DecisionRow> {
  const { userId } = await requireCanAct(id);
  const row = await transitionOrFriendlyError(id, 'cancelled', userId);
  revalidatePath('/');
  return row;
}
